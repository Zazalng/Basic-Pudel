...
Pudel's Category Management is a plugin module for the Pudel Discord Bot that provides a full Components v2 control panel for creating, importing, managing permissions, and unlinking Discord categories via slash command. It automates the assignment of category-level permission overrides for designated managers and default roles, keeping your server organized and secure.
...
* **Default Permission Settings:** Configure default Allow / Inherit / Deny permission states for both Manager and Default Role using a retro NES-style cursor navigation UI (⬆️ Up, ⬇️ Down, 🔄 Swap) (We're short component limit for had every button for each permission for swap).
...
* **Permission Auto-Apply:** When creating or importing, Pudel automatically applies permission overrides based on your configured defaults. Setting a Default Role makes the category private (denies `VIEW_CHANNEL` for @everyone).
...
* **Database Persistence:** All tracked categories, permission settings, and privilege roles are stored in Pudel's plugin database for persistence across restarts.
...
* **⚙️ Setting** — View and edit default permissions for Manager and Default Role, manage Privilege Roles
...
Navigate with **⬆️ Up** / **⬇️ Down**, toggle with **🔄 Swap** (cycles Inherit → Allow → Deny → Inherit), then **✅ Confirm** to save or **🔃 Reset** to reload from database.
...
### Management
Manage Channel · Manage Permission · Manage Webhooks · Manage Events · Create Events · Create Invite · Use App Commands · Use Activities · Use External Apps

### Messaging
Send Message & Posts · Create Public Threads · Use External Emojis · Use External Stickers · Send in Threads & Posts · Create Private Threads · Embed Links · Attach Files · Add Reactions · Mention @everyone · Manage Messages · Manage Threads & Posts · Read Message History · Send TTS Message · Send Voice Message · Create Polls

### Voice
Connect · Speak · Video · Use Soundboard · Use External Sounds · Use Voice Activity · Priority Speaker · Mute Members · Deafen Members · Move Members · Set Voice Status · Request to Speak
...
*Note: You have a window-time for responding to components within 15 min (any interaction refreshes the respond time), or your session will expire and you must start again.*
